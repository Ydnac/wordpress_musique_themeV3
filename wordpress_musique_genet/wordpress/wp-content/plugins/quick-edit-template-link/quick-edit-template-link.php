...
function qetl_textarea_field_0_render() {
	...
	$options = get_option( 'qetl_settings' );
	?>
	<input type='text' name='qetl_settings[qetl_max_recursive]' value='<?php echo $options['qetl_max_recursive']; ?>'> (0 = unlimited)
	<?php
}
...
function qetl_checkbox_field_1_render() {
	...
	foreach ( $plugins as $key => $val ) {
		...
		?>
		<input type='checkbox' name='qetl_settings[qetl_exclude_plugin_<?php echo $hash; ?>]' <?php checked( $options[ 'qetl_exclude_plugin_' . $hash ], 1 ); ?> value='1'>
		...
		<div class="wrap">
			...
			<div style="width:60%;float:left;">
				...
				<h2 class="nav-tab-wrapper">
					<a class="nav-tab <?php echo (($active_tab == 'general') ? 'nav-tab-active' : NULL); ?>" href="admin.php?page=quick_edit_template_link"><?php esc_attr_e('General'); ?></a>
					<a class="nav-tab <?php echo (($active_tab == 'child_theme') ? 'nav-tab-active' : NULL); ?>" href="admin.php?page=quick_edit_template_link&tab=child_theme"><?php esc_attr_e('Create a Child Theme'); ?></a>
				</h2>
...
function doThemeCreateForm()
{
    ...
    ?>
        <div id="" class="updated">
            <p>Your Child theme has been created, head over to <a href="<?php echo admin_url('themes.php'); ?>">Themes</a> to activate it</p>
        </div>
        <div id="" style="border-left:solid 4px #7ad03a; background-color:#fff; box-shadow: 0 1px 1px 0 rgba(0,0,0,.1);">
            <p style="margin: 0;padding: 13px 17px;">Your Child theme has been created, head over to <a href="<?php echo admin_url('themes.php'); ?>">Themes</a> to activate it</p>
        </div>
